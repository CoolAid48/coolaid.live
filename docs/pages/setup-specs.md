# 🖥️ CoolAid's Setup Specs 🖥️
#### One organized list containing all of CoolAid's PC specs and streaming gear

---
### 🎮 Main Streaming Desktop 🎮
- *CPU* - Core I9-13900K 
- *GPU* - RTX 4070 
- *Motherboard* - ASUS Z790-PLUS WIFI
- *RAM* - G.Skill Trident 64GB DDR5 - 6000mhz - CL36
- *Case* - Lian Li CG237
- *CPU Cooler* - ARCTIC Liquid Freezer III Pro 360 A-RGB
- *Main Storage* - Samsung 980 Pro 1 TB SSD
- *Additional Storage* - Crucial P3 Plus 2 TB SSD | WD Black SN850x 1 TB SSD
- *Power Supply* - Corsair RM850x

| <img src="/fishTank.png" alt="Main PC" width="420" /> |
| --- |
| Main Streaming PC -- Lian Li CG237 Case -- "Fish Tank"

### 🎙️ Peripherals & Streaming Gear 🎙️
- *Main Monitor* - LG Ultragear 27GN65S - 27"
- *Secondary Monitors* - x2 Koorui 24E4 - 24" | Old LG 22LJ4540 LED TV - 22"
- *Elgato Stream Decks* - Stream Deck XL | Stream Deck+ (w/ USB Hub)
- *Mouse* - Logitech G502 Hero
- *Keyboard* - Keychron S1 Low-Profile
- *Headset* - HyperX Cloud III
- *Microphone* - Shure SM7b XLR Microphone (w/ Elgato Mic Arm)
- *Audio Mixer* - TC-Helicon GoXLR Mini

| ![Peripherals Setup 1](/keyboard.png) |
| --- |
| Keychron S1 -- QMK Mechanical Keyboard -- Low-Profile -- Red Switches |

### 💻 Miscellaneous Gear and Secondary Laptop 💻
- [HP Pavilion Gaming Laptop](https://www.walmart.com/ip/HP-Pavilion-16-FHD-144Hz-Gaming-Laptop-i7-10750H-8GB-512GB-GTX-1650-Ti/900353214)
- *Custom Laptop RAM* - TEAMGROUP 32GB DDR4
- *Secondary Mouse* - Steelseries Rival 5
- *Alternate Mic* - TONOR TC40 USB Condenser Microphone
- *Deskpad* - [CoolAid Desk Mat](https://shop.coolaid.live/products/coolaid-desk-mat)